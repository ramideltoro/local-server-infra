...
for(const id of ids){const provider=staticSites.sites[id]?staticSites:backendApps.ids.includes(id)?backendApps:netlifyApps.ids.includes(id)?netlifyApps:firebaseApps.ids.includes(id)?firebaseApps:workers.ids.includes(id)?workers:raspberry.ids.includes(id)?raspberry:cloudWorkers.ids.includes(id)?cloudWorkers:localApps;if(!staticSites.sites[id]&&!localApps.ids.includes(id)&&!backendApps.ids.includes(id)&&!netlifyApps.ids.includes(id)&&!firebaseApps.ids.includes(id)&&!workers.ids.includes(id)&&!raspberry.ids.includes(id)&&!cloudWorkers.ids.includes(id))throw Error('Unknown application');
 ...
 if(!process.argv.includes('--force')&&prior?.testedRevision===revision&&Date.now()-Date.parse(prior.verifiedAt)<(prior.outcome==='pass'?(provider.intervalMs||7*86400000):6*3600000))continue;
 ...
 try{
  ...
  if(provider.verifyBackup)await provider.verifyBackup(id,work);
  if(await provider.revision(id,credentials.GITHUB_TOKEN)!==revision)throw Error('Deployment changed during backup verification');
  tests={'archive-integrity':true,'required-components':true};const at=new Date().toISOString();evidence.systems[id].readiness={outcome:'pass',testedRevision:revision,backupReference,verifiedAt:at,expiresAt:new Date(Date.parse(at)+(provider.expiryMs||8*86400000)).toISOString(),tests:{...tests}};await publish();
  await writable(work);if(provider.prepare)await provider.prepare(id,work);
  if(provider.restore){await provider.restore(id,work);
 ...
 const at=new Date().toISOString();const result={outcome:failure?'fail':'pass',testedRevision:revision,backupReference,verifiedAt:at,expiresAt:new Date(Date.parse(at)+(provider.expiryMs||8*86400000)).toISOString(),tests};
 if(provider.restore)evidence.systems[id].restore=result;else evidence.systems[id].readiness=result;evidence.systems[id].observedAt=at;
