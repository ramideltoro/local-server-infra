import {isolatedWorkerEnvironment} from './worker-environment.mjs';
...
assert(['scheduler','fetcher','canonicalizer','enrichment','approval','translation','persistence','publication'].includes(name));
...
try{
 ...
 const schema='worker_uplift_'+name;const table=name==='scheduler'?'feed_leases':'inbox';const count=Number(sql('SELECT count(*) FROM '+schema+'.'+table));assert(count>0,'Actual restored worker state required');
 let row=name==='scheduler'?null:JSON.parse(sql("SELECT row_to_json(x) FROM (SELECT * FROM "+schema+".inbox WHERE status='processed' ORDER BY id DESC LIMIT 1) x"));
 let storedCarrier;
 if(['persistence','translation'].includes(name)){
  const saved=sql("SELECT json_build_object('row',row_to_json(i),'carrier',o.diagnostic_metadata) FROM "+schema+".inbox i JOIN (SELECT idempotency_key,diagnostic_metadata FROM worker_uplift_translation.outbox UNION ALL SELECT idempotency_key,diagnostic_metadata FROM worker_uplift_approval.outbox) o USING (idempotency_key) WHERE i.status='processed' AND jsonb_typeof(o.diagnostic_metadata->'payload')='object' AND o.diagnostic_metadata->'envelope'->>'messageId'=i.message_id::text ORDER BY i.id DESC LIMIT 1");
 ...
 if(name==='scheduler'){
  const queue='nutsnews.worker.fetch.v1';let queued=0;
  for(let n=0;n<120;n++){queued=(await channel.checkQueue(queue)).messageCount;if(queued>0)break;await pause(500);}
  assert(queued>0,'Restored scheduler must publish work from actual restored feeds');
  assert(Number(sql('SELECT count(*) FROM '+schema+'.feed_leases'))>count,'Restored scheduler must persist new leases');
  await fs.writeFile('/work/result.json',JSON.stringify({worker:name,restoredLeaseRows:count,readiness:true,publishedMessages:queued,scope:'Isolated scheduling from restored feed and lease state'}));
 }else{
 ...
 const fixture=contracts.STAGE_PAYLOAD_FIXTURES.find(f=>contracts.validateStagePayload(f.payload).definition?.consumer===route.consumer);assert(fixture,'Replay carrier schema required');let payload={...fixture.payload,pipelineRunId:row.pipeline_run_id,sourceMessageId:row.source_message_id,idempotencyKey:row.idempotency_key};if(storedCarrier){envelope=storedCarrier.envelope;payload=storedCarrier.payload;assert.equal(envelope.idempotencyKey,row.idempotency_key);assert(contracts.validateWorkerEnvelope(envelope).ok);if(name==='persistence'){const {sha256Digest}=await import('/work/app/dist/src/digest.js');assert.equal(sha256Digest({aggregate:envelope.aggregate,payload}),row.payload_digest,'Original backed-up message fingerprint must match');}else{assert.equal(envelope.messageId,row.message_id);assert.equal(envelope.payloadRef.uri,row.payload_ref);assert.equal(envelope.aggregate.id,row.entity_id);if(envelope.payloadRef.digest)assert.equal(envelope.payloadRef.digest,row.payload_digest,'Original stored reference digest must match');}}assert(contracts.validateStagePayload(payload).ok);
 ...
 if(connection){await channel.close();await connection.close();connection=null;}
