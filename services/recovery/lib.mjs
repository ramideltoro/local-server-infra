...
export async function sandbox(work,command,{network=false,timeout=600000,postgres=false,broker=false}={}) {
  ...
  if(broker){const at=a.findIndex((v,i)=>v==='--symlink'&&a[i+1]==='usr/lib');a.splice(at,3,'--tmpfs','/lib','--ro-bind','/usr/lib/x86_64-linux-gnu','/lib/x86_64-linux-gnu','--ro-bind',work+'/broker/lib/ld-musl-x86_64.so.1','/lib/ld-musl-x86_64.so.1');for(const name of ['erlang','openssl','rabbitmq'])a.push('--ro-bind',work+'/broker/opt/'+name,'/opt/'+name);}
